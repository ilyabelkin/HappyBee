...
* HappyBee also **fixes a bug when Heat Recovery Ventilator (HRV) and furnace fan run indefinitely** when the native "Away for now" mode is triggered remotely via the ecobee app, API, pushover.net etc.
...
* **Trigger pushover.net API Webhooks** when a critical event occurs
* **Rate-limit** non-critical messages
* **Turn on and off additional smart switches** to work around Wemo limitations
...
* **messenger** is a special bee that knows how to talk to humans (via email or pushover.net Webhooks, allowing phone/VOIP calls, text, and more)
